...
Matrix::Matrix(double* data, ArrayType array_type, int n, int m, bool is_view):
m_data(data), m_multiplier(1) ,m_array_type(array_type), m_row_dim(n), m_col_dim(m), m_stride(m), m_is_view(is_view) {}
...
Matrix::Matrix(const Matrix& that) : Matrix(NULL, that.get_mpi_interpretation(), that.get_row_dimension(), that.get_col_dimension(), false) {
...
Matrix Matrix::sub_matrix(int num_rows, int num_col, int start_row, int start_col) {
	...
	return Matrix(get_data(start_row, start_col), m_array_type, num_rows, num_col, true);
}
